...
/* ----- input parameters-------------- */
	...
	iso_time_spacing = atoi(argv[16])*1000; // input ms conver to us
	repeat_iso = atoi(argv[17]);
	// new added to control ch2 output amplititude @ 2021/01/25 by Tina
	ch2_final_amp = atof(argv[18])/1000; //input mV convert to V 
...
/*-------trapping start-----------*/		
	...
	/*-------isolation -----------*/   
		...
		for (int i =0; i < repeat_iso ; i++)
		{
			
			rp_GenAmp(RP_CH_2, 1);	
			usleep(ISOLATION_TIME*1000-50);
			rp_GenAmp(RP_CH_2, 0);
			usleep(iso_time_spacing);
		}
		pin_write( TEST_TTL_1, 0); //isolation trigger
	...
	/*-------ramp -----------*/ 
		...
		ramp_ch2 = 0;
		ramp_step2 = ch2_final_amp/ramp_pts;
