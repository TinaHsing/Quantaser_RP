...
/* ----- input parameters-------------- */
	...
	ramp_step = atof(argv[9])/1000;//input mV convert to V 
	ramp_pts = atol(argv[10]);
	final_amp = atof(argv[11])/1000;//input mV convert to V 
	offset = atof(argv[12])/1000;//input mV convert to V 
	adc_offset = atoi(argv[13]);
