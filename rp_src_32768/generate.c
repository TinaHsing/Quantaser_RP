...
int generate_getFrequency(rp_channel_t channel, float *frequency) {
    volatile ch_properties_t *ch_properties;
    getChannelPropertiesAddress(&ch_properties, channel);
    *frequency = (float) round((ch_properties->counterStep * DAC_FREQUENCY) / (65536LU * BUFFER_LENGTH)); 
    return RP_OK;
}
...
int generate_getBufferReadPoint(rp_channel_t channel, uint32_t *value){
	volatile ch_properties_t *ch_properties;
    getChannelPropertiesAddress(&ch_properties, channel);
    *value = ch_properties->buffReadPointer;
    return RP_OK;	
}
...
int generate_writeData(rp_channel_t channel, float *data, uint32_t start, uint32_t length) {
	...
	// printf("properties:%p\n", properties);
	// printf("buffReadPointer:%p\n\n", &(properties->buffReadPointer));
    for(int i = start; i < start+BUFFER_LENGTH; i++) {
		...
		// data_cmn = cmn_CnvVToCnt(DATA_BIT_LENGTH, data[i-start], AMPLITUDE_MAX, false, amp_max, dc_offs, 0.0);
		// fprintf(fp, "%d, %d, %p, %f, %d, %d, %d\n",wo_idx, i, dataOut+i, data[i], data_cmn, *(dataOut+i), properties->buffReadPointer);
    }
	// fclose(fp);
	// printf("\n\n\n");
    return RP_OK;
